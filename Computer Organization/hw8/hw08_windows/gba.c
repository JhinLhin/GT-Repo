...
*/
void setPixel(int row, int col, u16 color) { // ROW and COL are the starting address of video buffer!!!
...
*/
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) { // ROW and COL are the starting address of video buffer!!!
...
void drawFullScreenImageDMA(const u16 *image) { 
  for (int i = 0; i <= HEIGHT - 1; i++){
    DMA[3].src = &image[OFFSET(i, 0, WIDTH)]; // image is stored consectively as an array, we increment row by row and each time copys over an row
...
*/
void drawImageDMA(int row, int col, int width, int height, const u16 *image) { // ROW and COL are the starting address of video buffer!!!
  for (int i = 0; i <= height - 1; i++){ // copying an image with a given height row by row
    DMA[3].src = &image[OFFSET(i, 0, width)];
    DMA[3].dst = &videoBuffer[OFFSET(row + i, col, WIDTH)]; // we only specified the address of first element
    // cnt copy a number of element at the starting address of each row
    DMA[3].cnt = width | DMA_ON | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
  }
}
...
/*
  ...
  This function can be completed using `height` DMA calls.
*/
// this function only draw a portion of image not the whole image
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) { // ROW and COL are the starting address of video buffer!!!
  for (int i = 0; i <= height - 1; i++){
    DMA[3].src = &image[OFFSET(i, 0, width)];
