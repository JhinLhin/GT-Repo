...
public class Operations
{
     ...
     */
    //0xFF
    int getByte(int num, int which) {return (num >> (which << 3)) & 0xFF;}
     ...
     */
    int swapShorts(int num) {return (num << 16) | ((num >> 16) & 0xFFFF);}
     ...
     */
    //(num | (0xF << (which << 2))) & (a_nibble << (which << 2));
    // this is wrong
    /* if
    num = 10010001, nibble = 1001, we first set num to 10011111
    then we do 10011111 and 00001001 we will get 00001001 instead of 10011111
     */
    /*
    instead if num = 101 10 011, nibble = 01
    we first make 000 11 000, and invert it, so we have 111 00 111
    then we do 111 00 111 and 101 10 011 = 101 00 011, then we do 101 00 011 or 000 01 000
    end up with 101 01 011
     */
    int setNibble(int num, int a_nibble, int which) {return  (num & ~(0xF << (which << 2))) | (a_nibble << (which << 2));}
     ...
     */
    // 8 16 8
    int pack(int b2, int s1, int b0) {return ((b2 & 0xFF) << 24) |(((s1 & 0xFFFF) << 8) | (b0 & 0xFF));}
     ...
     */
    //s included, but n is not
    // 4 * 8 = 32
    //return (((num >> s) << (32 | (~n | 1))) >> (32 | (~n | 1))) & (n | (~s | 1));
    //00001010  12 左 6
    //10000000  右 6
    //11111110
    //11111110 and 00000011

    //11111111
    //start it the starting index, n is the length of the ans, it is not the end index!!!!!!
...
//note it have to be positive
    boolean powerOf2(int num) {return num > 0 && (num & (num - 1)) == 0;}}
